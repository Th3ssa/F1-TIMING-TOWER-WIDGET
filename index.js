// ═══════════════════════════════════════════════════════════════════
//  F1 LIVE TIMING TOWER — SillyTavern Extension v4.1.0
//  Author  : Th3ssa
...
function parseMessage(msgIndex) {
    ...
    // Strategy 1: standard markdown render class
    let found = false;
    $msg.find('code.language-f1timing').each(function () {
        const $pre = jQuery(this).closest('pre');
        $pre.hide();
    ...
    // Strategy 2: any code element whose class contains f1timing
    ...
    // Strategy 3: scan ALL pre/code blocks for JSON with "drivers" key
    $msg.find('pre').each(function () {
        const $pre  = jQuery(this);
        const $code = $pre.find('code');
        const raw   = ($code.length ? $code : $pre).text().trim();
        if (raw.includes('"drivers"') && raw.includes('"flag"') && raw.startsWith('{')) {
            $pre.hide();
            tryParse(raw);
            found = true;
            return false; // break
    ...
    // Strategy 4: regex scrape raw message HTML for the JSON block
    const html = $msg.find('.mes_text').html() || '';
    const match = html.match(/```f1timing\s*([\s\S]*?)```/);
    if (match) {
        // Hide any visible code block
        $msg.find('pre').each(function () {
            if (jQuery(this).text().includes('"drivers"')) jQuery(this).hide();
        });
...
function tryParse(raw) {
    ...
    try {
        ...
        if (data && Array.isArray(data.drivers)) {
            applyData(data);
            console.log('[F1 Tower] ✅ Data parsed, drivers:', data.drivers.length);
        }
    } catch(e) {
        console.warn('[F1 Tower] JSON parse failed:', e.message, '| raw:', raw.slice(0, 80));
    }
}
...
function injectTower() {
    ...
    handle.addEventListener('mousedown',e=>{if(e.target.tagName!=='BUTTON'){start(e.clientX,e.clientY);e.preventDefault();}});
    handle.addEventListener('touchstart',e=>{if(e.target.tagName!=='BUTTON'){const t=e.touches[0];start(t.clientX,t.clientY);}},{passive:true});
...
console.log('[F1 Timing Tower v4.1.0] ✅ Loaded — github.com/Th3ssa/F1-TIMING-TOWER-WIDGET');
